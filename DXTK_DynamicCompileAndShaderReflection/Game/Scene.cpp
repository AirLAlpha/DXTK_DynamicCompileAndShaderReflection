...
Scene::Scene(DX::DeviceResources* pDeviceResources) :
    ...
    m_selectedModelType(ModelType::GeometricPrimitive),
    m_selectedILType(InputLayoutType::GeometricPrimitive),
    m_metallic(0.0f),
    m_smoothness(0.5f),
    m_useAlbedo(true),
    m_useNormalMap(true)
...
void Scene::Render()
{
    ...
    // PBRシェーダー用の定数バッファを更新
    {
        ...
        if (pbrCB != nullptr)
        {
            ...
            // テクスチャを使う
            pbrCB->variableData["t_useAlbedoTexture"].pArrayStart[0] = m_useAlbedo;
            // 法線マップを使う
            pbrCB->variableData["t_useNormalMap"].pArrayStart[0] = m_useNormalMap;
            // 金属
            pbrCB->variableData["f_metallic"].pArrayStart[0] = m_metallic;
            // なめらかさ
            pbrCB->variableData["f_smoothness"].pArrayStart[0] = m_smoothness;
        }
    }
    ...
    // モデルの描画
    if(m_selectedModelType == ModelType::GeometricPrimitive)
    {
        // ジオメトリックプリミティブの描画
...
void Scene::InitializeShaders()
{
    ...
    if (m_selectedILType == InputLayoutType::CMO)
    {
        macros = { 
            {"MODELTYPE_CMO", ""},
            {NULL, NULL} 
...
void Scene::InitializeModels()
{
    ...
    {
    case ModelType::GeometricPrimitive:
        // ジオメトリックプリミティブの作成
...
void Scene::UpdateDebugGUI()
{
    ...
    if (ImGui::BeginCombo("ModelType", modelList[(int)m_selectedModelType]))
    {
        for (int i = 0; i < 3; i++)
        {
            const bool selected = i == (int)m_selectedModelType;
            if (ImGui::Selectable(modelList[i], selected))
            {
                m_selectedModelType = (ModelType)i;
                InitializeModels();
            }
        }
        ImGui::EndCombo();
    ...
    // インプットレイアウトタイプ
    if (ImGui::BeginCombo("InputLayoutType", modelList[(int)m_selectedILType]))
    {
        for (int i = 0; i < 2; i++)
        {
            const bool selected = i == (int)m_selectedILType;
            if (ImGui::Selectable(modelList[i], selected))
            {
                m_selectedILType = (InputLayoutType)i;
                InitializeShaders();
            }
        }
        ImGui::EndCombo();
    ...
    // シェーダーパラメーター
    ImGui::SeparatorText("PBR Params");
    ...
    // アルベドテクスチャの使用
    ImGui::Checkbox("Use AlbedoTexture", &m_useAlbedo);
    // 法線マップの使用
    ImGui::Checkbox("Use NormalMap", &m_useNormalMap);


    ImGui::Spacing();

    // 再コンパイルボタン
    if (ImGui::Button("Recompile Shaders", { -1, 20 }))
    {
        InitializeShaders();
    }
